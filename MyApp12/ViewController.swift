...
class ViewController: UIViewController {
    ...
    let queue3 = DispatchQueue(label: "tw.org.iii.q3", qos: DispatchQoS.userInteractive)
           //userInteractive 為最高優先執行
    let queue4 = DispatchQueue(label: "tw.org.iii.q4", attributes: .concurrent) //concurrent 併行執行
...
//        }
        ...
        queue4.async {         //async 非同步 但在相同的queue裡 是按照順序執行 在不同的queue裡會同時進行
            self.job1()        //sync 同步 為按照順序進行
        }
        queue4.async {
            self.job2()
        }
